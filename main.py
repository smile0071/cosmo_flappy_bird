...
red = (255, 0, 0)
yellow = (255, 255, 0)
pygame.display.set_caption('Cosmo Flappy Bird')
...
# music
pygame.mixer.init()
pygame.mixer.music.load('Mysterious Strange Things - Yung Logos.mp3')
...
def draw_player(x_pos, y_pos):
    ...
    if y_change < 0:
        ...
        flame1_1 = pygame.draw.rect(screen, red, [x_pos - 18, y_pos + 30, 3, 18], 0, 2)
        flame2 = pygame.draw.rect(screen, yellow, [x_pos - 10, y_pos + 29, 7, 20], 0, 2)
        flame1_2 = pygame.draw.rect(screen, red, [x_pos - 8, y_pos + 30, 3, 18], 0, 2)
    return play
...
def draw_obstacles(obst, y_pos, play):
    ...
    for i in range(len(obst)):
        y_coord = y_pos[i]
        top_rect = pygame.draw.rect(screen, gray, [obst[i], 0, 30, y_coord - 200])
        top2 = pygame.draw.rect(screen, gray, [obst[i] - 3, y_coord - 220, 36, 20], 0, 5)
        bot_rect = pygame.draw.rect(screen, gray, [obst[i], y_coord, 30, HEIGHT])
        bot2 = pygame.draw.rect(screen, gray, [obst[i] - 3, y_coord, 36, 20], 0, 5)
...
while running:
    ...
    screen.blit(high_score_text, (10, 13))
